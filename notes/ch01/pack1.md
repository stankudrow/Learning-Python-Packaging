...
Довольно длинная простыня, но в итооге создано два обменопригодных архива (обав в диреткории "pack1/dist"):

1. колесо (wheel) - это что любит pip и считается основным форматом упаковки Python проекта
2. архив .tar.gz, который называется source distribution (sdist), который наверное можно коряво перевести как "исходниковый раздаток" (ужас!) - ну пусть будет "издаток" если у нас ~~импорт~~ввозозамещение - его тоже полезно распоространять с колесом, обойдёмся пока этим объяснением.
...
Можно установить также из архимва ".tar.gz":
```shell
